...
CSVNAME	     = 'company_aboutus.csv'			  # Store company pages here
...
def main():
	...
	myCSVwriter(CSVNAME,['TICKER','NAME']+[j for i in [('LINK '+str(i) , 'TEXT '+str(i)) for i in range(1,4)] for j in i],1)
	...
	for c,t in comp_names:
		...
		results = get_results(cq,"about us")

		LINKSU  = [i['href'] for i in results]

		# Collect text data for these links until we have 6 websites in buffer
		collect_buffer = []
		counter	       = 0
		while (len(collect_buffer) < min(3,len(LINKSU))) & (counter < len(LINKSU)):
